...
namespace forge
{
    namespace common
    {
        float clampTo01(const float pValue) { return (pValue < 0.0f ? 0.0f : (pValue > 1.0f ? 1.0f : pValue)); }
...
#    include <vector>
        ...
        void getFontFilePaths(std::vector<string>& pFiles, const string& pDir, const string& pExt)
        {
            WIN32_FIND_DATA ffd;
            LARGE_INTEGER   filesize;
            TCHAR           szDir[MAX_PATH];
            size_type       length_of_arg;
            DWORD           dwError = 0;
        ...
        {
            size_type             len = std::to_string(pVal).length();
        ...
        glm::vec3 trackballPoint(const float pX, const float pY, const float pWidth, const float pHeight)
        {
