...
namespace forge
{
    Surface::Surface(unsigned pNumXPoints, unsigned pNumYPoints, dtype pDataType, PlotType pPlotType, MarkerType pMarkerType)
    {
        ...
        FG_THROW(fg_create_surface(&temp, pNumXPoints, pNumYPoints, (fg_dtype)pDataType, pPlotType, pMarkerType));
    ...
    void Surface::setColor(const float pRed, const float pGreen, const float pBlue, const float pAlpha) { FG_THROW(fg_set_surface_color(get(), pRed, pGreen, pBlue, pAlpha)); }

    void Surface::setLegend(const char* pLegend) { FG_THROW(fg_set_surface_legend(get(), pLegend)); }
