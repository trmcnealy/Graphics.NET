...
namespace forge
{
    namespace common
    {
        ...
        public:
            Image(const unsigned pWidth, const unsigned pHeight, const forge::ChannelFormat pFormat, const forge::dtype pDataType) :
                mImage(std::make_shared<detail::image_impl>(pWidth, pHeight, pFormat, pDataType))
            {
            }

            Image(const fg_image pOther) { mImage = reinterpret_cast<Image*>(pOther)->impl(); }
