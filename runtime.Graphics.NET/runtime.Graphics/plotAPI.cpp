...
fg_err fg_create_plot(fg_plot* pPlot, const unsigned pNPoints, const fg_dtype pType, const fg_chart_type pChartType, const fg_plot_type pPlotType, const fg_marker_type pMarkerType)
{
    try
    {
        ARG_ASSERT(1, (pNPoints > 0));

        *pPlot = getHandle(new common::Plot(pNPoints, (forge::dtype)pType, pPlotType, pMarkerType, pChartType));
    }
    CATCHALL
...
fg_err fg_set_plot_color(fg_plot pPlot, const float pRed, const float pGreen, const float pBlue, const float pAlpha)
{
    try
    {
        ARG_ASSERT(0, (pPlot != nullptr));
