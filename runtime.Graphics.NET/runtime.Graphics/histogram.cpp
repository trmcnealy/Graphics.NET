...
namespace forge
{
    ...
    void Histogram::setColor(const float pRed, const float pGreen, const float pBlue, const float pAlpha) { FG_THROW(fg_set_histogram_color(get(), pRed, pGreen, pBlue, pAlpha)); }

    void Histogram::setLegend(const char* pLegend) { FG_THROW(fg_set_histogram_legend(get(), pLegend)); }
