...
namespace forge
{
    ...
    void VectorField::setColor(const float pRed, const float pGreen, const float pBlue, const float pAlpha) { FG_THROW(fg_set_vector_field_color(get(), pRed, pGreen, pBlue, pAlpha)); }

    void VectorField::setLegend(const char* pLegend) { FG_THROW(fg_set_vector_field_legend(get(), pLegend)); }
