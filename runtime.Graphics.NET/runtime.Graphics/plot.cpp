...
namespace forge
{
    Plot::Plot(const unsigned pNumPoints, const dtype pDataType, const ChartType pChartType, const PlotType pPlotType, const MarkerType pMarkerType)
    {
        fg_plot temp = nullptr;
        FG_THROW(fg_create_plot(&temp, pNumPoints, (fg_dtype)pDataType, pChartType, pPlotType, pMarkerType));
    ...
    void Plot::setColor(const float pRed, const float pGreen, const float pBlue, const float pAlpha) { FG_THROW(fg_set_plot_color(get(), pRed, pGreen, pBlue, pAlpha)); }

    void Plot::setLegend(const char* pLegend) { FG_THROW(fg_set_plot_legend(get(), pLegend)); }

    void Plot::setMarkerSize(const float pMarkerSize) { FG_THROW(fg_set_plot_marker_size(get(), pMarkerSize)); }
