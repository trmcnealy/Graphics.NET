...
typedef cl_mem GfxResourceHandle;
    ...
    static void createGLBuffer(GfxHandle** pOut, const unsigned pResourceId, const BufferType pTarget)
    {
    ...
    static void releaseGLBuffer(GfxHandle* pHandle) { free(pHandle); }

    static void copyToGLBuffer(GfxHandle* pGLDestination, ComputeResourceHandle pSource, const size_type pSize)
    {
...
#    define FORGE_CUDA_CHECK(err) (handleCUDAError(err, __FILE__, __LINE__))
    ...
    static void createGLBuffer(GfxHandle** pOut, const unsigned pResourceId, const BufferType pTarget)
    {
        ...
        FORGE_CUDA_CHECK(cudaGraphicsGLRegisterBuffer(&cudaImageResource, pResourceId, cudaGraphicsMapFlagsWriteDiscard));
    ...
    static void releaseGLBuffer(GfxHandle* pHandle)
    {
...
#    define FORGE_OCL_CHECK(cl_status, message)                         \
    ...
    static void copyToGLBuffer(GfxHandle* pGLDestination, ComputeResourceHandle pSource, const size_type pSize)
    {
        ...
        fg_finish();
        FORGE_OCL_CHECK(clEnqueueAcquireGLObjects(queue, 1, &dst, 0, NULL, &waitEvent), "Failed in clEnqueueAcquireGLObjects");
        FORGE_OCL_CHECK(clWaitForEvents(1, &waitEvent), "Failed in clWaitForEvents after clEnqueueAcquireGLObjects");
        FORGE_OCL_CHECK(clEnqueueCopyBuffer(queue, src, dst, 0, 0, pSize, 0, NULL, &waitEvent), "Failed in clEnqueueCopyBuffer");
        FORGE_OCL_CHECK(clEnqueueReleaseGLObjects(queue, 1, &dst, 0, NULL, &waitEvent), "Failed in clEnqueueReleaseGLObjects");
        FORGE_OCL_CHECK(clWaitForEvents(1, &waitEvent), "Failed in clWaitForEvents after clEnqueueReleaseGLObjects");
    }
