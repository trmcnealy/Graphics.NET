...
fg_err fg_create_surface(fg_surface* pSurface, const unsigned pXPoints, const unsigned pYPoints, const fg_dtype pType, const fg_plot_type pPlotType, const fg_marker_type pMarkerType)
{
    try
    {
        ...
        *pSurface = getHandle(new common::Surface(pXPoints, pYPoints, (forge::dtype)pType, pPlotType, pMarkerType));
    }
    CATCHALL
...
fg_err fg_set_surface_color(fg_surface pSurface, const float pRed, const float pGreen, const float pBlue, const float pAlpha)
{
    try
    {
        ARG_ASSERT(0, (pSurface != nullptr));
