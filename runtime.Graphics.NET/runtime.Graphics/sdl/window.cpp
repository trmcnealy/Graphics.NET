...
namespace forge
{
    namespace wtk
    {
            ...
            mWindow(nullptr), mClose(false), mLastXPos(0), mLastYPos(0), mButton(-1)
        {
            ...
            mWindow = SDL_CreateWindow((pTitle != nullptr ? pTitle : "Forge-Demo"),
                                       SDL_WINDOWPOS_UNDEFINED,
                                       SDL_WINDOWPOS_UNDEFINED,
                                       pWidth,
                                       pHeight,
                                       (invisible ? SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN : SDL_WINDOW_OPENGL) | SDL_WINDOW_RESIZABLE);
            if(mWindow == NULL)
            {
        ...
        void Widget::makeContextCurrent() const { SDL_GL_MakeCurrent(mWindow, mContext); }
        ...
        GLADloadproc Widget::getProcAddr() { return static_cast<GLADloadproc>(SDL_GL_GetProcAddress); }

        void Widget::setTitle(const char* pTitle) { SDL_SetWindowTitle(mWindow, (pTitle != nullptr ? pTitle : "Forge-Demo")); }

        void Widget::setPos(int pX, int pY) { SDL_SetWindowPosition(mWindow, pX, pY); }

        void Widget::setSize(unsigned pW, unsigned pH) { SDL_SetWindowSize(mWindow, pW, pH); }
        ...
        void Widget::pollEvents()
        {
            ...
            while(SDL_PollEvent(&evnt))
            {
                ...
                if(evnt.key.windowID == mWindowId)
                {
                    ...
                    {
                        if(evnt.button.button == SDL_BUTTON_MIDDLE && (mMod == SDLK_LCTRL || mMod == SDLK_RCTRL))
                        {
