...
#include <glsl_shaders/image_fs.hpp>
#include <glsl_shaders/image_vs.hpp>
...
namespace forge
{
    namespace opengl
    {
        void image_impl::bindResources(int pWindowId) const { glBindVertexArray(screenQuadVAO(pWindowId)); }

        void image_impl::unbindResources() const { glBindVertexArray(0); }

        image_impl::image_impl(const uint32_t pWidth, const uint32_t pHeight, const forge::ChannelFormat pFormat, const forge::dtype pDataType) :
            mWidth(pWidth),
            mHeight(pHeight),
            mFormat(pFormat),
            mDataType(pDataType),
            ...
            mCMapIndex(-1)
        {
            ...
            glTexImage2D(GL_TEXTURE_2D, 0, ictype2gl(mFormat), mWidth, mHeight, 0, ctype2gl(mFormat), dtype2gl(mDataType), nullptr);

            CheckGL("Before PBO Initialization");
            glGenBuffers(1, &mPBO);
            glBindBuffer(GL_PIXEL_UNPACK_BUFFER, mPBO);
            size_type typeSize = 0;
            ...
            glBufferData(GL_PIXEL_UNPACK_BUFFER, mPBOsize, nullptr, GL_STREAM_COPY);
        ...
        void image_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
            ...
            glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, mWidth, mHeight, ctype2gl(mFormat), dtype2gl(mDataType), nullptr);
