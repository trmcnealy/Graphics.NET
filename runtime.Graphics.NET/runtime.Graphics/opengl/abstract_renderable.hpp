...
namespace forge
{
    namespace opengl
    {
        ...
        protected:
            ...
            AbstractRenderable() : mVBO(0), mCBO(0), mABO(0), mVBOSize(0), mCBOSize(0), mABOSize(0), mIsPVCOn(0), mIsPVAOn(0)
            {
                mColor[0] = 0;
                mColor[1] = 0;
                mColor[2] = 0;
        ...
        public:
            ...
            size_type vboSize() const { return mVBOSize; }
            size_type cboSize() const { return mCBOSize; }
            size_type aboSize() const { return mABOSize; }
             ...
             * the range of [0, 1]
             */
            void setColor(const float pRed, const float pGreen, const float pBlue, const float pAlpha)
            {
                mColor[0] = pRed;
                mColor[1] = pGreen;
                mColor[2] = pBlue;
             ...
             */
            void setRanges(const float pMinX, const float pMaxX, const float pMinY, const float pMaxY, const float pMinZ, const float pMaxZ)
            {
                mRange[0] = pMinX;
                mRange[1] = pMaxX;
                mRange[2] = pMinY;
             ...
             */
            virtual void setColorMapUBOParams(const uint32_t pUBO, const uint32_t pSize) {}
             ...
             * @return nothing.
             */
            virtual void render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient) = 0;
        };
