...
#include <glsl_shaders/histogram_vs.hpp>
    ...
    namespace opengl
    {
        void histogram_impl::bindResources(const int pWindowId)
        {
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glVertexAttribPointer(mPointIndex, 2, GL_FLOAT, GL_FALSE, 0, nullptr);
                // attach histogram frequencies
                glEnableVertexAttribArray(mFreqIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mVBO);
                glVertexAttribPointer(mFreqIndex, 1, dtype2gl(mDataType), GL_FALSE, 0, nullptr);
                glVertexAttribDivisor(mFreqIndex, 1);
                // attach histogram bar colors
                glEnableVertexAttribArray(mColorIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mCBO);
                glVertexAttribPointer(mColorIndex, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
                ...
                glVertexAttribPointer(mAlphaIndex, 1, GL_FLOAT, GL_FALSE, 0, nullptr);
...
#undef HIST_CREATE_BUFFERS
        ...
        void histogram_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
