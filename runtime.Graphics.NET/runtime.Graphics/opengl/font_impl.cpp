...
#include <runtime.Graphics/util.hpp>
#include <runtime.Graphics/opengl/font_impl.hpp>
#include <runtime.Graphics/opengl/gl_helpers.hpp>
#include <glsl_shaders/font_fs.hpp>
#include <glsl_shaders/font_vs.hpp>
...
#    define FT_THROW_ERROR(msg, err)                        \
        ...
        void font_impl::loadAtlasWithGlyphs(const size_type pFontSize)
        {
            FT_Library library;
            FT_Face    face;
            ...
            auto& currList = mGlyphLists[pFontSize - size_type(MIN_FONT_SIZE)];
            ...
            for(int i = START_CHAR; i <= END_CHAR; ++i)
            {
                ...
                bError = FT_Glyph_To_Bitmap(&currGlyph, FT_RENDER_MODE_NORMAL, nullptr, 1);
                if(bError)
                {
        ...
        void font_impl::bindResources(int pWindowId)
        {
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, GLsizei(2 * sz), nullptr);
        ...
        font_impl::font_impl() : mTTFfile(""), mIsFontLoaded(false), mAtlas(new FontAtlas(512, 512, 1)), mVBO(0), mProgram(glsl::font_vs.c_str(), glsl::font_fs.c_str()), mOrthoW(1), mOrthoH(1)
        {
            ...
            mGlyphLists.resize(size_type(MAX_FONT_SIZE - MIN_FONT_SIZE) + 1, GlyphList());
        }
        ...
        void font_impl::setOthro2D(size_type pWidth, size_type pHeight)
        {
            mOrthoW  = pWidth;
            mOrthoH  = pHeight;
        ...
        void font_impl::loadFont(const char* const pFile)
        {
            ...
            for(size_type s = size_type(MIN_FONT_SIZE); s <= size_type(MAX_FONT_SIZE); ++s)
            {
            ...
            for(size_type f = 0; f < mGlyphLists.size(); ++f)
            {
        ...
        void font_impl::render(int pWindowId, const float pPos[], const float pColor[], const char* pText, size_type pFontSize, bool pIsVertical)
        {
            ...
            {
                pFontSize = size_type(MIN_FONT_SIZE);
            }
            else if(pFontSize > MAX_FONT_SIZE)
            {
                pFontSize = size_type(MAX_FONT_SIZE);
            }

            glm::mat4 R = (pIsVertical ? glm::rotate(I, glm::radians(90.f), glm::vec3(0, 0, 1)) : I);

            auto& glyphList = mGlyphLists[pFontSize - size_type(MIN_FONT_SIZE)];

            for(size_type i = 0; i < std::strlen(pText); ++i)
            {
                int ccode = pText[i];
