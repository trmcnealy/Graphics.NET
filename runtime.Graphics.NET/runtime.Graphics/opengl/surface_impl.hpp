...
namespace forge
{
    namespace opengl
    {
        ...
        public:
            surface_impl(const uint32_t pNumXpoints, const uint32_t pNumYpoints, const forge::dtype pDataType, const forge::MarkerType pMarkerType);
            ~surface_impl();
            ...
            inline void usePerVertexColors(const bool pFlag = true) { mIsPVCOn = pFlag; }

            inline void usePerVertexAlphas(const bool pFlag = true) { mIsPVAOn = pFlag; }
        };
        ...
        public:
            scatter3_impl(const uint32_t pNumXPoints, const uint32_t pNumYPoints, const forge::dtype pDataType, const forge::MarkerType pMarkerType = FG_MARKER_NONE) :
