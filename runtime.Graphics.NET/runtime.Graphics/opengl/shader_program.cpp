...
Shaders loadShaders(const char* pVertexShaderSrc, const char* pFragmentShaderSrc, const char* pGeometryShaderSrc)
{
    uint32_t f, v;
    ...
    glShaderSource(v, 1, &pVertexShaderSrc, nullptr);
    glShaderSource(f, 1, &pFragmentShaderSrc, nullptr);
    ...
    if(pGeometryShaderSrc)
    {
        ...
        glShaderSource(g, 1, &pGeometryShaderSrc, nullptr);
        glCompileShader(g);
    ...
    namespace forge
    {
        namespace opengl
        {
            ShaderProgram::ShaderProgram(const char* pVertShaderSrc, const char* pFragShaderSrc, const char* pGeomShaderSrc) : mVertex(0), mFragment(0), mGeometry(0), mProgram(0)
            {
            ...
            uint32_t ShaderProgram::getUniformLocation(const char* pAttributeName) { return glGetUniformLocation(mProgram, pAttributeName); }

            uint32_t ShaderProgram::getUniformBlockIndex(const char* pAttributeName) { return glGetUniformBlockIndex(mProgram, pAttributeName); }

            uint32_t ShaderProgram::getAttributeLocation(const char* pAttributeName) { return glGetAttribLocation(mProgram, pAttributeName); }
