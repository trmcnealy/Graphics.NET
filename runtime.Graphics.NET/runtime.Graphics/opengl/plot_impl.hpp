...
namespace forge
{
    namespace opengl
    {
        ...
        public:
            plot_impl(const uint32_t pNumPoints, const forge::dtype pDataType, const forge::PlotType pPlotType, const forge::MarkerType pMarkerType, const int pDimension = 3);
            ~plot_impl();

            void setMarkerSize(const float pMarkerSize);

            uint32_t  markers();
            size_type markersSizes() const;

            virtual void render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient);
        };
        ...
        public:
            plot2d_impl(const uint32_t pNumPoints, const forge::dtype pDataType, const forge::PlotType pPlotType, const forge::MarkerType pMarkerType) :
