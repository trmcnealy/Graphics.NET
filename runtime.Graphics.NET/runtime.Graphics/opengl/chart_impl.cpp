...
#include <runtime.Graphics/opengl/chart_impl.hpp>
#include <runtime.Graphics/font.hpp>
#include <runtime.Graphics/opengl/font_impl.hpp>
#include <runtime.Graphics/opengl/gl_helpers.hpp>
#include <runtime.Graphics/opengl/histogram_impl.hpp>
#include <runtime.Graphics/opengl/image_impl.hpp>
#include <runtime.Graphics/opengl/plot_impl.hpp>
#include <glsl_shaders/chart_fs.hpp>
#include <glsl_shaders/chart_vs.hpp>
#include <glsl_shaders/tick_fs.hpp>
#include <runtime.Graphics/opengl/surface_impl.hpp>
...
namespace forge
{
    namespace opengl
    {
        ...
        void AbstractChart::renderTickLabels(const int                       pWindowId,
                                             const uint32_t                  pW,
                                             const uint32_t                  pH,
                                             ...
                                             const bool                      pUseZoffset) const
        {
            ...
            for(StringIter it = pTexts.begin(); it != pTexts.end(); ++it)
            {
                ...
                glm::vec4 p   = glm::vec4(mTickTextX[idx + pCoordsOffset], mTickTextY[idx + pCoordsOffset], (pUseZoffset ? mTickTextZ[idx + pCoordsOffset] : 0), 1);
        ...
        AbstractChart::AbstractChart(const float pLeftMargin, const float pRightMargin, const float pTopMargin, const float pBottomMargin) :
            mTickCount(9),
            mTickSize(10.0f),
        ...
        void AbstractChart::setAxesVisibility(const bool isVisible) { mRenderAxes = isVisible; }

        void AbstractChart::setAxesLimits(const float pXmin, const float pXmax, const float pYmin, const float pYmax, const float pZmin, const float pZmax)
        {
            mXMin = pXmin;
            mXMax = pXmax;
            mYMin = pYmin;
        ...
        void AbstractChart::setAxesLabelFormat(const std::string& pXFormat, const std::string& pYFormat, const std::string& pZFormat)
        {
        ...
        void AbstractChart::getAxesLimits(float* pXmin, float* pXmax, float* pYmin, float* pYmax, float* pZmin, float* pZmax)
        {
            *pXmin = mXMin;
            *pXmax = mXMax;
            *pYmin = mYMin;
        ...
        void AbstractChart::addRenderable(const std::shared_ptr<AbstractRenderable> pRenderable) { mRenderables.emplace_back(pRenderable); }
        ...
        void chart2d_impl::bindResources(const int pWindowId)
        {
            ...
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glVertexAttribPointer(mBorderAttribPointIndex, 2, GL_FLOAT, GL_FALSE, 0, nullptr);
                glBindVertexArray(0);
        ...
        void chart2d_impl::generateTickLabels()
        {
            ...
            // Y Axis label format
            if(toString(std::fabs(mYMax - mYMin), mYLabelFormat).length() > 5 && std::regex_search(mYLabelFormat, PRINTF_FIXED_FLOAT_RE))
            {
                mYLabelFormat = std::string("%.2e");
            }
            // X Axis label format
            if(toString(std::fabs(mXMax - mXMin), mXLabelFormat).length() > 5 && std::regex_search(mXLabelFormat, PRINTF_FIXED_FLOAT_RE))
            {
        ...
        chart2d_impl::chart2d_impl() : AbstractChart(0.13139f, 0.1008f, 0.0755f, 0.1077f)
        {
            generateChartData();
            generateTickLabels();
        }

        void chart2d_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
            ...
            glm::mat4 trans = glm::translate(glm::scale(glm::mat4(1), glm::vec3(scale_x, scale_y, 1)), glm::vec3(offset_x, offset_y, 0));
        ...
        void chart3d_impl::bindResources(const int pWindowId)
        {
            ...
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glEnableVertexAttribArray(mBorderAttribPointIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mDecorVBO);
                glVertexAttribPointer(mBorderAttribPointIndex, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
                glBindVertexArray(0);
                /* store the vertex array object corresponding to
                 * the window instance in the map */
                mVAOMap[pWindowId] = vao;
        ...
        void chart3d_impl::generateTickLabels()
        {
            ...
            // certain length, the numbers are converted to scientific notation.
            // Z Axis label format
            if(toString(std::fabs(mZMax - mZMin), mZLabelFormat).length() > 5 && std::regex_search(mZLabelFormat, PRINTF_FIXED_FLOAT_RE))
            {
        ...
        chart3d_impl::chart3d_impl() : AbstractChart(0.0933f, 0.03701f, 0.1077f, 0.0085f)
        {
            generateChartData();
            generateTickLabels();
        }

        void chart3d_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
            static const glm::mat4 VIEW       = glm::lookAt(glm::vec3(-1.0f, 0.5f, 1.0f), glm::vec3(1.0f, -1.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
            static const glm::mat4 PROJECTION = glm::ortho(-1.64f, 1.64f, -1.64f, 1.64f, -0.001f, 1000.f);
            static const glm::mat4 MODEL      = glm::rotate(glm::mat4(1.0f), -glm::radians(90.f), glm::vec3(0, 1, 0)) * glm::rotate(glm::mat4(1.0f), -glm::radians(90.f), glm::vec3(1, 0, 0));
            ...
            renderTickLabels(pWindowId, uint32_t(w), uint32_t(h), mZText, trgtFntSize, trans, 0);
            renderTickLabels(pWindowId, uint32_t(w), uint32_t(h), mYText, trgtFntSize, trans, mTickCount);
            renderTickLabels(pWindowId, uint32_t(w), uint32_t(h), mXText, trgtFntSize, trans, 2 * mTickCount);
