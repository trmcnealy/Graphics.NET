...
#include <glsl_shaders/histogram_fs.hpp>
#include <glsl_shaders/marker2d_vs.hpp>
#include <glsl_shaders/marker_fs.hpp>
#include <glsl_shaders/plot3_fs.hpp>
#include <glsl_shaders/plot3_vs.hpp>
    ...
    namespace opengl
    {
        void plot_impl::bindResources(const int pWindowId)
        {
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glVertexAttribPointer(mPlotPointIndex, mDimension, dtype2gl(mDataType), GL_FALSE, 0, nullptr);
                // attach colors
                glEnableVertexAttribArray(mPlotColorIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mCBO);
                glVertexAttribPointer(mPlotColorIndex, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
                // attach alphas
                glEnableVertexAttribArray(mPlotAlphaIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mABO);
                glVertexAttribPointer(mPlotAlphaIndex, 1, GL_FLOAT, GL_FALSE, 0, nullptr);
                // attach radii
                glEnableVertexAttribArray(mMarkerRadiiIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mRBO);
                glVertexAttribPointer(mMarkerRadiiIndex, 1, GL_FLOAT, GL_FALSE, 0, nullptr);
                glBindVertexArray(0);
        ...
        void plot_impl::bindDimSpecificUniforms() { glUniform2fv(mPlotRangeIndex, 3, mRange); }

        plot_impl::plot_impl(const uint32_t pNumPoints, const forge::dtype pDataType, const forge::PlotType pPlotType, const forge::MarkerType pMarkerType, const int pD) :
            mDimension(pD),
            mMarkerSize(12),
            ...
            mPlotType(pPlotType),
            mIsPVROn(false),
            mPlotProgram(pD == 2 ? glsl::marker2d_vs.c_str() : glsl::plot3_vs.c_str(), pD == 2 ? glsl::histogram_fs.c_str() : glsl::plot3_fs.c_str()),
...
#undef PLOT_CREATE_BUFFERS
        ...
        void plot_impl::setMarkerSize(const float pMarkerSize) { mMarkerSize = pMarkerSize; }
        ...
        size_type plot_impl::markersSizes() const { return mRBOSize; }

        void plot_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
        ...
        void plot2d_impl::bindDimSpecificUniforms() { glUniform4fv(mPlotUColorIndex, 1, mColor); }
