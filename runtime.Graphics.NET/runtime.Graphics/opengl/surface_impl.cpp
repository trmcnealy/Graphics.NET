...
namespace forge
{
    namespace opengl
    {
        void generateGridIndices(std::vector<unsigned int>& indices, unsigned short rows, unsigned short cols)
        {
        ...
        void surface_impl::bindResources(const int pWindowId)
        {
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glVertexAttribPointer(mSurfPointIndex, 3, dtype2gl(mDataType), GL_FALSE, 0, nullptr);
                glEnableVertexAttribArray(mSurfColorIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mCBO);
                glVertexAttribPointer(mSurfColorIndex, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
                glEnableVertexAttribArray(mSurfAlphaIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mABO);
                glVertexAttribPointer(mSurfAlphaIndex, 1, GL_FLOAT, GL_FALSE, 0, nullptr);
                // attach indices
        ...
        void surface_impl::renderGraph(const int pWindowId, const glm::mat4& transform)
        {
            ...
            glDrawElements(GL_TRIANGLE_STRIP, GLsizei(mIBOSize), GL_UNSIGNED_INT, (void*)nullptr);
            unbindResources();
            mSurfProgram.unbind();
            ...
            if(mMarkerType != FG_MARKER_NONE)
            {
                ...
                bindResources(pWindowId);
                glDrawElements(GL_POINTS, GLsizei(mIBOSize), GL_UNSIGNED_INT, (void*)nullptr);
                unbindResources();
        ...
        surface_impl::surface_impl(unsigned pNumXPoints, unsigned pNumYPoints, forge::dtype pDataType, forge::MarkerType pMarkerType) :
...
#undef SURF_CREATE_BUFFERS
        ...
        void surface_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
