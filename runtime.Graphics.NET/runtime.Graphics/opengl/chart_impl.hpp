...
#include <runtime.Graphics/opengl/abstract_renderable.hpp>
...
namespace forge
{
    namespace opengl
    {
        ...
        protected:
            ...
            inline float getTickStepSize(float minval, float maxval) const { return (maxval - minval) / (mTickCount - 1); }
            ...
            inline float getLeftMargin(int pWidth) const { return pWidth * mLeftMargin; }

            inline float getRightMargin(int pWidth) const { return pWidth * mRightMargin; }

            inline float getBottomMargin(int pHeight) const { return pHeight * mBottomMargin; }

            inline float getTopMargin(int pHeight) const { return pHeight * mTopMargin; }

            inline float getTickSize() const { return mTickSize; }

            void renderTickLabels(const int                       pWindowId,
                                  const uint32_t                  pW,
                                  const uint32_t                  pH,
        ...
        public:
            AbstractChart(const float pLeftMargin, const float pRightMargin, const float pTopMargin, const float pBottomMargin);
            virtual ~AbstractChart();

            void setAxesVisibility(const bool isVisible = true);

            void setAxesTitles(const char* pXTitle, const char* pYTitle, const char* pZTitle);

            void setAxesLimits(const float pXmin, const float pXmax, const float pYmin, const float pYmax, const float pZmin, const float pZmax);

            void setAxesLabelFormat(const std::string& pXFormat, const std::string& pYFormat, const std::string& pZFormat);

            void getAxesLimits(float* pXmin, float* pXmax, float* pYmin, float* pYmax, float* pZmin, float* pZmax);
        ...
        public:
            ...
            void render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient);
        };
