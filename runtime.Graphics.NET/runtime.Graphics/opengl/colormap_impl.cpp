...
#include <runtime.Graphics/opengl/colormap_impl.hpp>
#include <runtime.Graphics/cmap.hpp>
#include <runtime.Graphics/opengl/gl_helpers.hpp>

#define CREATE_UNIFORM_BUFFER(color_array, size) createBuffer(GL_UNIFORM_BUFFER, 4 * size, color_array, GL_STATIC_DRAW)
...
namespace forge
{
    namespace opengl
    {
        colormap_impl::colormap_impl()
        {
            ...
            size_type channel_bytes = sizeof(float) * 4; /* 4 is for 4 channels */
        ...
        colormap_impl::~colormap_impl() { glDeleteBuffers(ForgeNumColorMaps, mMapIds.data()); }

        uint32_t colormap_impl::cmapUniformBufferId(forge::ColorMap cmap) const { return mMapIds[static_cast<unsigned int>(cmap)]; }

        uint32_t colormap_impl::cmapLength(forge::ColorMap cmap) const { return mMapLens[static_cast<unsigned int>(cmap)]; }
