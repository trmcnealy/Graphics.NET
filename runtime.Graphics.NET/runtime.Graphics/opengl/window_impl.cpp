...
#    endif
    ...
    namespace opengl
    {
        window_impl::window_impl(int pWidth, int pHeight, const char* pTitle, std::weak_ptr<window_impl> pWindow, const bool invisible) : mID(getNextUniqueId())
        {
            ...
            {
                mWidget = widget_ptr(new wtk::Widget(pWidth, pHeight, pTitle, observe->get(), invisible));
            }
            else
            {
                /* when windows are not sharing any context, just create
                 * a dummy wtk::Widget object and pass it on */
                mWidget = widget_ptr(new wtk::Widget(pWidth, pHeight, pTitle, widget_ptr(), invisible));
            }
        ...
        void window_impl::setFont(const std::shared_ptr<font_impl>& pFont) { mFont = pFont; }
        ...
        void window_impl::setSize(unsigned pW, unsigned pH) { mWidget->setSize(pW, pH); }
        ...
        const std::shared_ptr<colormap_impl>& window_impl::colorMapPtr() const { return mCMap; }
        ...
        void window_impl::draw(const std::shared_ptr<AbstractRenderable>& pRenderable)
        {
            ...
            pRenderable->render(mID, 0, 0, mWidget->mWidth, mWidget->mHeight, viewMatrix, orientMatrix);
        ...
        void window_impl::draw(const int pRows, const int pCols, const int pIndex, const std::shared_ptr<AbstractRenderable>& pRenderable, const char* pTitle)
        {
            ...
            auto FIErrorHandler = [](FREE_IMAGE_FORMAT pOutputFIFormat, const char* pMessage) { printf("FreeImage Error Handler: %s\n", pMessage); };
            ...
            glReadPixels(0, 0, mWidget->mWidth, mWidget->mHeight, GL_RGBA, GL_UNSIGNED_BYTE, pbuf.data());
            uint32_t i = 0;
