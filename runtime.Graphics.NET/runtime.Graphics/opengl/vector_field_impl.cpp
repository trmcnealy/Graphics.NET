...
#include <glsl_shaders/vector_field2d_gs.hpp>
#include <glsl_shaders/vector_field2d_vs.hpp>
#include <glsl_shaders/vector_field_gs.hpp>
#include <glsl_shaders/vector_field_vs.hpp>
    ...
    namespace opengl
    {
        void vector_field_impl::bindResources(const int pWindowId)
        {
            if(mVAOMap.find(pWindowId) == mVAOMap.end())
            {
                ...
                glVertexAttribPointer(mFieldPointIndex, mDimension, dtype2gl(mDataType), GL_FALSE, 0, nullptr);
                // attach colors
                glEnableVertexAttribArray(mFieldColorIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mCBO);
                glVertexAttribPointer(mFieldColorIndex, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
                // attach alphas
                glEnableVertexAttribArray(mFieldAlphaIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mABO);
                glVertexAttribPointer(mFieldAlphaIndex, 1, GL_FLOAT, GL_FALSE, 0, nullptr);
                // attach field directions
                glEnableVertexAttribArray(mFieldDirectionIndex);
                glBindBuffer(GL_ARRAY_BUFFER, mDBO);
                glVertexAttribPointer(mFieldDirectionIndex, mDimension, GL_FLOAT, GL_FALSE, 0, nullptr);
                glBindVertexArray(0);
        ...
        glm::mat4 vector_field_impl::computeModelMatrix(const glm::mat4& pOrient)
        {
            ...
            glm::vec3 shiftVector(-(mRange[0] + mRange[1]) / 2.0f, -(mRange[2] + mRange[3]) / 2.0f, -(mRange[4] + mRange[5]) / 2.0f);
...
#undef PLOT_CREATE_BUFFERS
        ...
        size_type vector_field_impl::directionsSize() const { return mDBOSize; }

        void vector_field_impl::render(const int pWindowId, const int pX, const int pY, const int pVPW, const int pVPH, const glm::mat4& pView, const glm::mat4& pOrient)
        {
