...
namespace forge
{
    Image::Image(const unsigned pWidth, const unsigned pHeight, const ChannelFormat pFormat, const dtype pDataType) : mValue(nullptr)
    {
    ...
    void Image::setAlpha(const float pAlpha) { FG_THROW(fg_set_image_alpha(get(), pAlpha)); }

    void Image::keepAspectRatio(const bool pKeep) { FG_THROW(fg_set_image_aspect_ratio(get(), pKeep)); }
    ...
    void Image::render(const Window& pWindow, const int pX, const int pY, const int pVPW, const int pVPH) const { FG_THROW(fg_render_image(pWindow.get(), get(), pX, pY, pVPW, pVPH)); }
