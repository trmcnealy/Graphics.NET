...
#include <cmath>
#include <runtime.Graphics/compute_copy.h>
...
static const size_type DATA_SIZE    = (size_type)((FRANGE_END - FRANGE_START) / dx);

__global__ void simple_sinf(float* out, const size_type _data_size, const int fnCode, const float _dx, const float _frange_start)
{
    const int i = blockIdx.x * blockDim.x + threadIdx.x;

    if(i < _data_size)
    {
        const float x   = _frange_start + i * _dx;
        const int   idx = 2 * i;
        out[idx]        = x;
...
inline int divup(const int a, const int b) { return (a + b - 1) / b; }

void kernel(float* dev_out, const int functionCode)
{
...
int main(int argc, char* argv[])
{
    ...
    forge::Plot plt1 = chart.plot(DATA_SIZE, forge::f32, FG_PLOT_LINE,
                                  FG_MARKER_NONE); // or specify a specific plot type
    forge::Plot plt2 = chart.plot(DATA_SIZE, forge::f32, FG_PLOT_LINE,
